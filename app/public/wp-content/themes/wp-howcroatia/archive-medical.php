<?php get_header(); ?>
  ...
  <section class="service-articles">
    <div class="container mx-auto mb-16">
      <h2 class="section-heading pb-14">Medical Services</h2>
...
<!-- SERVICES FORM -->
  <form class="" action="<?php echo site_url() ?>/wp-admin/admin-ajax.php" method="POST" id="medical_service">
  <div class="filter-selectors__filter filter-selectors__filter--light">
  <div class="filter-selectors__filter__label">Service:</div>
    <?php
      if( $terms = get_terms( array( 'taxonomy' => 'service-medical', 'orderby' => 'name' ) ) ) : 
  
        echo '<select class="hotel__select" name="medicalService"><option value="">All Services</option>';
    ...
    ?>
    <input type="hidden" name="action" value="serviceMedicalFilter">
    </div>
  </form>
 
<!-- LOCATION FORM -->
  <form class="" action="<?php echo site_url() ?>/wp-admin/admin-ajax.php" method="POST" id="medical_location">
  <div class="filter-selectors__filter filter-selectors__filter--light">
  <div class="filter-selectors__filter__label">Location:</div>
    <?php
      if( $terms = get_terms( array( 'taxonomy' => 'location-medical', 'orderby' => 'name' ) ) ) : 
  
        echo '<select class="hotel__select" name="medicalLocation"><option value="">All Locations</option>';
        foreach ( $terms as $term ) :
          echo '<option value="' . $term->term_id . '">' . $term->name . '</option>'; // ID of the category as the value of an option
        endforeach;
        echo '</select>';
    ...
    ?>
    <input type="hidden" name="action" value="locationMedicalFilter">
    </div>
  </form>
 </div>
 </div>
   <div class="container__howcroatia" id="medical_response">
